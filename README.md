...
| Destructive rm (system) | `rm -rf /etc*`, `rm -rf /usr*`, `rm -rf /var*`, `rm -rf /home*`, `rm -rf /Applications*`, `rm -rf /System*`, `rm -rf /Library*`, `rm -rf /bin*`, `rm -rf /sbin*`, `rm -rf /Users*` (and `-fr` variants) | System directories |
...
- `Bash(rm *)` is allowed for local project use but guarded by deny rules for system directories (`/`, `/System`, `/Library`, `/bin`, `/sbin`, `/Users`, `/etc`, `/usr`, `/var`, `/home`, `/Applications`, `~`, `.git`). The deny list may not cover all dangerous paths.
